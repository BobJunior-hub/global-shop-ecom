  ...
  return (
    <div className="w-full max-w-sm bg-white rounded-2xl border border-zinc-200 p-8 flex flex-col gap-6 shadow-sm">
      <div className="flex flex-col gap-1">
        <h1 className={`text-xl font-bold ${theme.primaryText}`}>Xush kelibsiz</h1>
      ...
      <form onSubmit={async (e) => { e.preventDefault(); await handleAction(new FormData(e.currentTarget)); }} className="flex flex-col gap-4">
        ...
        <Input
          ...
          rightElement={
            <button
              ...
              className={`${theme.text} hover:text-zinc-700 transition-colors`}
              tabIndex={-1}
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full mt-1 py-2 rounded-lg ${theme.primary} ${theme.primaryHover} text-white text-sm font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2`}
        >
      ...
      <p className="text-sm text-center text-zinc-500">
        Hisob yo'qmi?{" "}
        <Link href={from !== "/products" ? `/register?from=${from}` : "/register"} className={`font-medium ${theme.primaryText} hover:underline`}>
          Ro'yxatdan o'tish
        </Link>
      </p>
    </div>
