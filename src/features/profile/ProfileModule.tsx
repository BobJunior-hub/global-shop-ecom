  ...
  if (!user) {
    return (
      <div className="max-w-3xl mx-auto w-full px-4 py-20 flex flex-col items-center gap-6">
        ...
        <Link
          href="/login?from=/"
          className="px-8 py-3 rounded-xl bg-zinc-900 text-white text-sm font-semibold hover:bg-zinc-800 transition-colors"
        >
          Kirish
        </Link>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-4 py-10 flex flex-col gap-8">
      ...
      <div className="flex items-center gap-5">
        <div className="w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center text-white text-xl font-bold shrink-0">
          {initials}
        </div>
        <div>
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center justify-between py-2 text-sm text-zinc-700 hover:text-green-700 transition-colors group"
              >
                {item.label}
                <svg className="w-4 h-4 text-zinc-300 group-hover:text-green-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
