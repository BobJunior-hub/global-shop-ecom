    ...
    <div className={`flex gap-4 py-4 border-b border-zinc-100 last:border-0 transition-opacity ${mutating ? "opacity-50 pointer-events-none" : ""}`}>
      ...
      <div className="flex flex-col gap-1 flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-0.5">
          ...
          {variant?.stock != null && (
            <span className={`text-xs font-medium ${variant.stock > 0 ? "text-green-600" : "text-red-500"}`}>
          ...
          {promoStatus === "applied" ? (
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="h-7 w-28 rounded-lg border border-zinc-200 bg-zinc-50 px-2 text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-400 focus:bg-white transition-all"
              />
              <button
                type="button"
                onClick={handleApplyPromo}
                disabled={promoStatus === "checking" || !promoInput.trim()}
                className="h-7 px-2.5 rounded-lg bg-zinc-900 text-white text-xs font-semibold hover:bg-zinc-700 disabled:opacity-40 transition-colors"
              >
