export type BrandKey = "albaraka" | "pijamapro" | "kidsworld";
...
export type BrandTheme = {
  primary: string;       // button bg,            e.g. "bg-black"
  primaryHover: string;  // button hover bg,      e.g. "hover:bg-zinc-900"
  primaryText: string;   // primary color as text, e.g. "text-zinc-900"
  primaryHex: string;    // raw hex for non-Tailwind usage, e.g. "#000000"
  iconBg: string;        // icon/badge/avatar bg,  e.g. "bg-[#dbc5c3]"
  text: string;          // secondary text,        e.g. "text-[#dbc5c3]"
...
const BRAND_CONFIGS: Record<BrandKey, BrandConfig> = {
  ...
  kidsworld: {
    key: "kidsworld",
    storeId: "7b7da4ea-e4c7-4c1f-8756-d8a804b2cc57",
    displayName: "Kids World",
    logoPath: "/kidsworld/logo.png",
    description: "Sifatli va chiroyli bolalar kiyimlari",
    metadataTitle: "Kids World - Onlayn do'kon",
    metadataDescription: "Kids World onlayn do'konida bolalar mahsulotlarini qulay va tez buyurtma qiling.",
    theme: {
      primary: "bg-[#eb4034]",
      primaryHover: "hover:bg-[#d63028]",
      primaryText: "text-[#eb4034]",
      primaryHex: "#eb4034",
      iconBg: "bg-[#dbc5c3]",
      text: "text-[#dbc5c3]",
      accent: "text-[#dbc5c3]",
      accentHover: "hover:bg-[#eb4034]",
    },
    locations: [],
    socials: [
      { label: "Instagram", href: "#", path: INSTAGRAM_PATH },
    ...
    icons: {
      icon:       "/kidsworld/favicon.ico",
      icon16:     "/kidsworld/favicon-16x16.png",
      icon32:     "/kidsworld/favicon-32x32.png",
      apple:      "/kidsworld/apple-touch-icon.png",
      android192: "/kidsworld/android-chrome-192x192.png",
      android512: "/kidsworld/android-chrome-512x512.png",
    },
  },
};
...
export function getBrandByHost(host?: string | null): BrandConfig {
  ...
  if (normalized.includes("kidsworld")) {
    return BRAND_CONFIGS.kidsworld;
  }
